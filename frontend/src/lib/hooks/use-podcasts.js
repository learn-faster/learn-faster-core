...
import { podcastsApi } from '@/modules/open-notebook/lib/api/podcasts'
...
import {
  ...
  speakerUsageMap,
} from '@/modules/open-notebook/lib/types/podcasts'

function hasActiveEpisodes(episodes) {
...
export function useDeletePodcastEpisode() {
  ...
  return useMutation({
    mutationFn: (episodeId) => podcastsApi.deleteEpisode(episodeId),
    onSuccess: () => {
...
export function useDeleteEpisodeProfile() {
  ...
  return useMutation({
    mutationFn: (profileId) => podcastsApi.deleteEpisodeProfile(profileId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.episodeProfiles })
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.podcastEpisodes })
      toast({
...
export function useSpeakerProfiles(episodeProfiles) {
  const query = useQuery({
...
export function useDeleteSpeakerProfile() {
  ...
  return useMutation({
    mutationFn: (profileId) => podcastsApi.deleteSpeakerProfile(profileId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.speakerProfiles })
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.episodeProfiles })
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.podcastEpisodes })
